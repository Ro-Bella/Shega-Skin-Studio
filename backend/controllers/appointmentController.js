...
exports.createAppointment = async (req, res) => { // አዲስ ቀጠሮ መፍጠር
    try { // የሚከናወን ኮድ
        ...
        // ማሻሻያ፡ ያለፈ ቀን ላይ ቀጠሮ እንዳይያዝ መከላከል
        const appointmentDate = new Date(`${date}T${timeSlot.replace(' PM', '').replace(' AM', '')}`); // This assumes specific format, simplified check below:
        const checkDate = new Date(date);
        const today = new Date();
        today.setHours(0, 0, 0, 0);

        if (checkDate < today) {
             return res.status(400).json({ success: false, message: 'እባክዎ ትክክለኛ የቀጠሮ ቀን ይምረጡ (ያለፈ ቀን አይቻልም)' });
        }
